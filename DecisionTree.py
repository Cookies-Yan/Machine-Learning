...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.majority_class = max(class_counts(rows).items(), key = operator.itemgetter(1))[0]
        self.rows = rows
        self.depth = depth
        self.id = id
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print (spacing + "Predicted ", node.majority_class)
        return

    # Print the question at this node
    print (spacing + str(node.question))
    # Print the depth, node id of the decision nodes
    print(spacing + "Node Depth: " + str(node.depth) + "Node ID: " + str(node.id))
    # Call this function recursively on the true branch
    print (spacing + '--> True:')
...
def getInnerNodes(node, innerNodes =[]):
    ...
    getLeafNodes(node.true_branch, innerNodes)
    getLeafNodes(node.false_branch, innerNodes)
